...
function App() {
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    const handler = e => {
      const c = document.querySelector(".live-bg-container");
      if (!c) return;
      c.style.setProperty("--mouse-x", `${(e.clientX / window.innerWidth) * 100}%`);
      c.style.setProperty("--mouse-y", `${(e.clientY / window.innerHeight) * 100}%`);
    };
  ...
  const handleSignOut = useCallback(async () => {
    ...
    setUser(null); setVideoHistory([]); setShowAuth(false);
  }, []);

  const addToHistory = useCallback(async (video) => {
    const entry = {
      media_id: video.videoId, title: video.title,
      source_type: video.sourceType || "youtube",
      viewed_at: new Date().toISOString(),
    };
    setVideoHistory(prev => [entry, ...prev.filter(v => v.media_id !== entry.media_id)].slice(0, 50));
    if (user?.user_id) {
      apiFetch("/history", {
        method: "POST",
        body: JSON.stringify({ user_id: user.user_id, media_id: video.videoId, title: video.title, source_type: video.sourceType || "youtube" }),
      }).catch(() => {});
    }
  }, [user]);
        ...
        <main className="main-content">
          ...
          <Routes>
            ...
            <Route path="/history" element={
              <History videoHistory={videoHistory} setCurrentVideo={setCurrentVideo} user={user} />
            } />
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      <div className="modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="modal-sub">{tab === "login" ? "Sign in to sync your sessions." : "Start analysing content with AI."}</div>
        ...
        <div className="auth-fields">
          {tab === "register" && (
            <div className="field-group">
              <label>Name</label>
              <input value={name} onChange={e => setName(e.target.value)} placeholder="Your name" onKeyDown={handleKey} autoFocus />
            </div>
          )}
          <div className="field-group">
            <label>Email</label>
            <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="you@example.com" onKeyDown={handleKey} autoFocus={tab === "login"} />
          </div>
          <div className="field-group">
            ...
            <div style={{ position: "relative" }}>
              ...
              <button
                ...
                style={{ position: "absolute", right: "0.7rem", top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "var(--text-tertiary)", fontSize: "0.82rem", padding: "4px" }}
                tabIndex={-1}
          ...
          {tab === "register" && (
            <div className="field-group">
              ...
              <input type={showPw ? "text" : "password"} value={pw2} onChange={e => setPw2(e.target.value)} placeholder="Repeat password" onKeyDown={handleKey} />
            </div>
          )}
        </div>

        {error && (
          <div style={{ color: "#f87171", fontSize: "0.84rem", marginBottom: "0.75rem", padding: "0.5rem 0.75rem", background: "rgba(248,113,113,0.09)", borderRadius: "8px", border: "1px solid rgba(248,113,113,0.22)" }}>
            {error}
          </div>
        )}

        <button className="btn-auth-primary" onClick={submit} disabled={busy}>
          {busy ? (tab === "register" ? "Creating…" : "Signing in…") : (tab === "register" ? "Create account" : "Sign in")}
        </button>
